...
- Story 2.1: (A) developer can load/edit a move and receives complete inline validation/conflict results; (B) developer saves and reloads one path-confined move with byte-preserving failure proof; (C) developer performs editor selection, save, UndoRedo, scaffold, and runtime-load flow in Godot.
- Story 2.2: (A) developer stages edits while active gameplay remains unchanged; (B) one writer commits and a stale concurrent writer receives a recoverable conflict with fault-equivalence proof; (C) developer observes committed tuning after watcher processing and restart in Godot.
- Story 2.3: (A) developer observes isolated per-attacker count/damage for authoritative hits, including invalid/overflow preservation; (B) developer observes deterministic hit/block/end and lifecycle resets without duplicate subscriptions; (C) rendered Godot display matches the ViewModel through exit/re-entry.
- Story 2.4: (A) developer records, names, lists, and reloads a validated immutable recording through the ViewModel/codec boundary; (B) developer plays the recording once from different absolute frames with identical relative outcomes; (C) developer starts/stops/loops while capture/live-input/Replay conflicts and lifecycle cancellation are visibly enforced; (D) Godot dummy playback produces the accepted deterministic hash window.
- Story 2.5: (A) developer saves a complete inspectable training snapshot containing every required participant and recording metadata; (B) developer atomically overwrites/loads a save while injected file/codec failures preserve the prior save and live state; (C) developer restores a mid-combo state through full Prepare/no-fail Commit and observes exactly one StateRestored/F+1 continuation; (D) developer restores assigned/mid-playback Story 2.4 recording state without old-epoch work; (E) Godot ViewModel flow reproduces the accepted multi-frame state hash.

## Approval Record

The binding decisions are stored in `_bmad-output/implementation-artifacts/evidence/v2-prep-2-4/role-approvals.md`. Any rejection reopens the affected row. Approvals apply only to the exact SHA-256 inventory in that evidence directory.
