...
- The current move-dataset schema version is `1`. Its canonical top-level envelope contains explicitly present `schema_version: 1` and non-null `moves` fields. Startup, editor load/save, hot reload, and runtime load reject missing, null, non-integral, older, or newer schema versions; ordinary load never performs migration.
- The persistence unit is one schema-version-1 move-dataset JSON document containing the complete `moves` collection; editing one move rewrites and atomically replaces that complete document while preserving every other compatible move. `move_id` is data identity and is never a filesystem path or filename source.
